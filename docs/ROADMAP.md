...
- [x] Recipe validation errors that name the file, section, and agent index
      — surfaced cleanly in the CLI: `bake run`, `bake report`, and `bake plan`
      fail with one clean `<cmd>: <message>` line instead of a traceback;
      missing recipe files get the same one-line treatment.
